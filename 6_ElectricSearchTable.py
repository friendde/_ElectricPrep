...
# fGDB variables
searchTable = r'C:\arcdata\transfer\MIMS_Electric_Extract.gdb\MIMS\mmElectricSearch'
...
with arcpy.da.Editor(workspace) as edit:
    for fc in fcFacilityID:
        ...
        with arcpy.da.SearchCursor(connectionRoot%(fc),fldsFacilityID) as sc:
            for scrow in sc:
                row = ((checkValue(scrow[0]),changeAliasName(fc),checkValue(scrow[1]),None,None,changeAliasName(fc)+'-'+checkValue(scrow[0]),scrow[2]))
                ic.insertRow(row)
        del sc
        del ic
